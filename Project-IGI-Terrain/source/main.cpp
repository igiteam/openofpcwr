...
// save as raw
constexpr int MENU_EXPORT_TO_RAW_4K = 61;
constexpr int MENU_EXPORT_TO_RAW_8K = 62;
constexpr int MENU_EXPORT_TO_RAW_16K = 63;

// exit
constexpr int MENU_CLOSE = 71;
...
static void OnMenu(int menu) {
	...
	case MENU_TERRAIN_DISCARD_MOD:
		...
		g_update_menu_flags |= UPDATE_MENU_TERRAIN_MODIFIER_OPTS;
		break;
	case MENU_EXPORT_TO_RAW_4K:
		g_app.ExportTerrainToRaw(Level::export_size_t::SIZE_4K);
		break;
	case MENU_EXPORT_TO_RAW_8K:
		g_app.ExportTerrainToRaw(Level::export_size_t::SIZE_8K);
		break;
	case MENU_EXPORT_TO_RAW_16K:
		g_app.ExportTerrainToRaw(Level::export_size_t::SIZE_16K);
		break;
	case MENU_CLOSE:
		glutLeaveMainLoop();
		break;
...
# endif
	...
	g_menu_export = glutCreateMenu(OnMenu);
	glutAddMenuEntry("raw format (4k)", MENU_EXPORT_TO_RAW_4K);
	glutAddMenuEntry("raw format (8k)", MENU_EXPORT_TO_RAW_8K);
	glutAddMenuEntry("raw format (16k)", MENU_EXPORT_TO_RAW_16K);
	...
	glutAddSubMenu("Export", g_menu_export);
