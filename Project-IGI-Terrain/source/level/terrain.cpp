...
bool Terrain::LoadCTRFile(load_params_s & params) {
	...
	for (int i = 0; i < ctr_file_contents.num_item_; ++i) {
		...
		for (int j = 0; j < 8; ++j) {
			dst->children_[j] = Swap_LitToInt16(src->children_[j]);
		}

		// link to cmd item
		dst->cmd_ = (cmd_item_s*)((char*)cmd_ + Swap_LitToUInt32(src->cmd_offset_));
	}
...
bool Terrain::LoadBITFile(load_params_s & params) {
	...
	for (int i = 0; i < MAX_TEX_MOD; ++i) {
		uint32_t sz = Swap_LitToUInt32(head[i].size_);
		if (sz) {
...
bool Terrain::GetZ(const dyn_cube_s* root_dyn_cube, const glm::vec3& pos, float & ret_z) {
	...
	float fx = pos.x - (float)get_z_ix_;
	float fy = pos.y - (float)get_z_iy_;

	get_z_is_int_pos_ = (std::fabs(fx) < 0.001f) && (std::fabs(fy) < 0.001f);
